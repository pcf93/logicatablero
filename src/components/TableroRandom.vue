<template>
  ...
  size: number;
    ...
    const generarFiguras = (cantidad: number): {rowIndex: number, colIndex: number, direction: string, size: number}[] => {
...
const asignarClaseOcupado = () => {
    ...
    const figuras = generarFiguras(3);
    const cuadros = generarCuadros(4);
    
    const elementos = figuras.concat(cuadros);
